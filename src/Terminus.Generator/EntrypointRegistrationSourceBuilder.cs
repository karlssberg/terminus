        ...
        string implementationClassName)
    {
        ...
        var registrationCollection = matchingEntryPoints.Select(ep =>
        {
            ...
            var registrationExpressionStatement =
                $"services.AddSingleton<EntryPointDescriptor<{attributeTypeName}>>(new EntryPointDescriptor<{attributeTypeName}>(typeof({containingType}).GetMethod(\"{methodName}\", {paramArray})!, context => {invokeExpression}));";
